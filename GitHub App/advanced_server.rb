  ...
  post '/' do
    ...
    when 'push'
      ...
      if payload['commits'][0]['distinct'] === true
        handle_commit_pushed(payload)
      end
    when 'gollum'
      handle_wiki_edited(payload)
    end
...
#
  ...
  helpers do
    ...
    def handle_commit_pushed(payload)
      pusher = payload['pusher']['name']
      addedFiles = payload['commits'][0]['added']
      removedFiles = payload['commits'][0]['removed']
      modifiedFiles = payload['commits'][0]['modified']
      commitURL = payload['commits'][0]['url']

      changedFile = commitURL + "/" + modifiedFiles[0]

      fileContentsEnc = @bot_client.contents(payload['repository']['id'], :path => modifiedFiles[0])['content']
      fileContents = Base64.decode64(fileContentsEnc)
      logger.debug "File Contents: " + fileContents

      lineCount = countLines(fileContents)
      logger.debug "Lines in class: " + lineCount.to_s

      logger.debug "The files " + "'#{modifiedFiles.join("','")}'" + " were modified and pushed by " + pusher
      true
    end

    def handle_wiki_edited(payload)
      if payload['pages'][0]['page_name'] === "Susereum"
        proposer = payload['sender']['login']
        logger.debug "The Susereum wiki was edited by " + proposer

        pageContents = payload['pages']['html_url']
